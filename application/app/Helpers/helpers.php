...
function importStatesToDB(){
    $states_db = [];
    foreach (getAllStatesRaw() as $value) {
        $state = [];
        $state["state_name"] = $value["name"];
        $state["state_code"] = $value["state_code"];
        $state["state_type"] = $value["type"];
        $state["created_at"] = date("Y-m-d H:i:s");
        $states_db[] = $state;
    }
    States::truncate();
    $state_insert = States::insert($states_db);
    if ($state_insert) {
...
function getStateByCode($state){
    $state_full = States::where("state_code", $state)->first();
    if ($state_full) {
        return $state_full["state_name"];
    }
    return null;
}
function getStateCodeByName($state){
    $state_full = States::where("state_name", $state)->first();
    if ($state_full) {
        return $state_full["state_code"];
    }
    return null;
}
function csv_to_array($delimiter = ',') {
    $filename = public_path() . '/csv.csv';
